...
export function NotesPage() {
  ...
  async function fetchQuestionPapers() {
    ...
    try {
      ...
      const fetchedQPs: QuestionPaper[] = (data.data || []).map(
        (item: any) => ({
          id: item.name,
          title: item.title,
          subject: item.subject,
  ...
  return (
    <div className="min-h-screen bg-gray-50 pb-14 md:pb-16">
      {/* Hero Section */}
      <section className="bg-academy-black text-white py-14 md:py-16 relative overflow-hidden">
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 text-center relative z-10">
          ...
          <h1 className="text-3xl sm:text-4xl md:text-5xl font-heading font-bold mb-3 md:mb-4 animate-fade-up">
            Study Materials
          </h1>
          <p className="text-base sm:text-lg text-gray-300 max-w-2xl mx-auto animate-fade-up delay-100 leading-relaxed">
            Access our comprehensive collection of notes and question papers to
            excel in your studies.
          </p>
        </div>
      </section>

      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 -mt-7 md:-mt-8 relative z-20">
        <Tabs defaultValue="notes" className="w-full">
          <TabsList className="grid w-full max-w-md mx-auto grid-cols-2 h-12 md:h-14 bg-white shadow-lg rounded-full p-1 mb-8 md:mb-10 animate-fade-up delay-200">
            <TabsTrigger
              value="notes"
              className="rounded-full text-sm md:text-base font-semibold data-[state=active]:bg-[#FFB902] data-[state=active]:text-white data-[state=inactive]:text-gray-600 data-[state=inactive]:hover:bg-gray-100 transition-all duration-300"
            >
              Notes
            </TabsTrigger>
            <TabsTrigger
          ...
          <TabsContent
            value="notes"
            className="space-y-6 md:space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-500"
          >
            {/* Filters */}
            ...
            {loadingNotes ? (
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                {[1, 2, 3].map((i) => (
                  <Card
                    key={i}
                    className="h-48 animate-pulse bg-gray-100 border-none"
                  />
                ))}
              </div>
            ) : errorNotes ? (
              <div className="text-center py-20">
                ...
                <Button variant="outline" onClick={fetchNotes} className="mt-4">
                  Retry
                </Button>
              </div>
            ) : notes.length === 0 ? (
              ...
              <>
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5 md:gap-6">
                  {notes.map((note, idx) => (
                    <Card
                      key={note.id}
                      className="group hover:shadow-2xl transition-all duration-300 hover:-translate-y-1 border-none shadow-md overflow-hidden glass"
                      style={{ animationDelay: `${idx * 100}ms` }}
                    >
                      <CardHeader className="bg-gradient-to-br from-orange-50 to-white border-b border-orange-100 p-5 md:p-6">
                        <div className="flex justify-between items-start">
                          <Badge
                            variant="secondary"
                            className="bg-white/80 backdrop-blur text-academy-orange hover:bg-white shadow-sm border-orange-100"
                          >
                            {note.subject}
                          </Badge>
                          <FileText className="h-5 w-5 text-gray-400 group-hover:text-academy-orange transition-colors" />
                        </div>
                        <CardTitle className="mt-4 text-lg md:text-xl leading-snug line-clamp-2 group-hover:text-academy-orange transition-colors">
                          {note.title}
                        </CardTitle>
                      </CardHeader>
                      <CardContent className="p-5 md:p-6">
                        <p className="text-sm text-gray-500 mb-5 line-clamp-3 leading-relaxed">
                          {note.description ||
                            "Comprehensive update for standard " +
                              note.standard}
                        </p>
                        <Button
                          ...
                          onClick={() =>
                            handleDownload(note.downloadUrl, note.title)
                          }
                        >
                ...
                <div className="flex justify-center items-center gap-3 md:gap-4 mt-7 md:mt-8">
                  <Button
                    variant="outline"
              ...
              <>
                <div className="grid gap-4">
                  {questionPapers.map((qp, idx) => (
                    <Card
                      key={qp.id}
                      className="group hover:shadow-lg transition-all duration-300 border-none shadow-sm glass overflow-hidden"
                      style={{ animationDelay: `${idx * 100}ms` }}
                    >
                      <div className="p-6 flex flex-col md:flex-row items-center justify-between gap-4">
                        <div className="flex items-center gap-4 w-full md:w-auto">
                          ...
                          <div>
                            <h3 className="font-bold text-lg text-academy-black group-hover:text-academy-orange transition-colors">
                              {qp.title}
                            </h3>
                            <div className="flex flex-wrap gap-2 mt-1">
                              ...
                              <Badge className="bg-academy-black text-xs">
                                {qp.examType}
                              </Badge>
                            </div>
                          </div>
                        ...
                        <Button
                          ...
                          onClick={() =>
                            handleDownload(qp.downloadUrl, qp.title)
                          }
                        >
